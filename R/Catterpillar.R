                        ...
                        plotlabels = TRUE, xMOR = 3, yMOR = 2, set.MOR = NULL, col.group=NULL) 
{
  ...
  plot <- ggplot(data = plot.df, aes(x = grp, y = condval, 
                                     ymin = lo, ymax = hi, col=col.group)) + geom_pointrange() + geom_hline(yintercept = 0, 
                                                                                                            linetype = 2) + coord_flip() + xlab(label = grp.var.t) + 
